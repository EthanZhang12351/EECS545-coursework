...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        x_embed_trans = x_embed.transpose(1, 0, 2)  # (T-1, N, W)
        h, cache_rnn = rnn_forward(x_embed_trans, h0, Wx, Wh, b)  # h: (T-1, N, H)
        h = h.transpose(1, 0, 2)  #  (N, T-1, H)

        scores, cache_temporal_fc = temporal_fc_forward(h, W_vocab, b_vocab)  # (N, T-1, V)
        ...
        dx_embed_trans, dh0, dWx, dWh, db = rnn_backward(dh_trans, cache_rnn)
        grads['Wx'] = dWx
        grads['Wh'] = dWh
        grads['b'] = db

        dx_embed = dx_embed_trans.transpose(1, 0, 2)
