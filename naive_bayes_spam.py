...
def train_naive_bayes(X: np.ndarray, Y: np.ndarray,
    ...
    ###########################################################################
    phi = np.sum(Y) / num_mails
    spam_mails = np.sum(X[Y == 1, :], axis = 0)
    total_spam = np.sum(X[Y == 1, :],)
    mu_spam = (spam_mails + 1) / (total_spam + vocab_size) 
    real_mails = np.sum(X[Y == 0, :], axis = 0)
    total_real = np.sum(X[Y == 0, :],)
    mu_non_spam = (real_mails + 1) / (total_real + vocab_size)
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
...
def test_naive_bayes(X: np.ndarray,
    ...
    ###########################################################################
    spam_scores = X.dot(np.log(mu_spam)) + np.log(phi)
    nonspam_scores = X.dot(np.log(mu_non_spam)) + np.log(1 - phi)
    for i in range(len(pred)):
        if spam_scores[i] > nonspam_scores[i]:
             pred[i] = 1
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
...
def get_indicative_tokens(mu_spam: np.ndarray,
    ...
    ###################################################################
    log_odds = np.log(mu_spam) - np.log(mu_non_spam)
    idx_list = np.argsort(log_odds)[:-top_k-1:-1]
    ###################################################################
    #                        END OF YOUR CODE                         #
    ###################################################################
    return idx_list
