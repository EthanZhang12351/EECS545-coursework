...
def conv_forward(x, w):
    ...
    for j in range(H_prime):
        for i in range(W_prime):
            ...
            # Broadcast w to shape (1, F, C, HH, WW) and x_slice to (N, 1, C, HH, WW)
            # Then sum over (C, HH, WW) to get shape (N, F).
            out[:, :, j, i] = np.sum(
                x_slice[:, None, :, :, :] * w[None, :, :, :, :],
                axis=(2, 3, 4)
            )
...
def conv_backward(dout, cache):
    ...
    for f in range(F):
        for c in range(C):
            for j in range(HH):
                for i in range(WW):
                    # Compute the gradient of the loss with respect to the filter
                    dw[f, c, j, i] = np.sum(dout[:, f, :, :] * x[:, c, j:j+H_prime, i:i+W_prime])
    for n in range(N):
        for f in range(F):
            for j in range(H_prime):
                for i in range(W_prime):
                    # Perform the backward pass
                    dx[n, :, j:j+HH, i:i+WW] += dout[n, f, j, i] * w[f, :, :, :]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw
