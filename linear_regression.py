...
def batch_gradient_descent(
    ...
    *,
    ...
    for current_epoch_number in range(max_epochs):
        ...
        ###################################################################
        gradient = ((X_train @ w - y_train).T @ X_train).T
        w = w - eta * gradient
        ###################################################################
        #                        END OF YOUR CODE                         #
        ###################################################################
...
def stochastic_gradient_descent(
    ...
    for current_epoch_number in range(max_epochs):
        for x_data_point, y_data_point in zip(X_train, y_train):
            ...
            ###################################################################
            gradient = (x_data_point @ w - y_data_point) * x_data_point
            w = w - eta * gradient
            ###################################################################
            #                        END OF YOUR CODE                         #
            ###################################################################
...
def compute_y_space(
    ...
    for idx, x_point in enumerate(x_space):
        ...
        #########################################################################
        r_train = np.exp(-((x_train - x_point) ** 2) / (2 * tau ** 2))
        w = closed_form_locally_weighted(X_train, y_train, r_train)
        y_space[idx] = np.dot(w, np.array([1, x_point]))
        #########################################################################
        #                          END OF YOUR CODE                             #
        #########################################################################
