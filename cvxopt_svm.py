...
# ~~START DELETE~~
This is adapted from Xavier Bourret Sicotte's tutorial:
...
def get_qp_params(X, y, C, kernel_params):
    ...
    q = -np.ones(n_samples).astype(np.float64)
    I = np.eye(n_samples)
    G = np.vstack([-I,I]).astype(np.float64)
    h = np.append(np.zeros(n_samples), C * np.ones(n_samples))
    h = h.astype(np.float64)
    A = np.array([y]).copy() 
    A = A.astype(np.float64)
    b = np.zeros(1).astype(np.float64)
    ###################################################################################
    #                                END OF YOUR CODE                           #######
    ...
    assert len(q.shape) == len(h.shape) == len(b.shape) == 1, 'q, h, and b must be vector'     
    ############################################################################s'
...
def decision_function(X, X_train, y_train, b, alpha, kernel_params):
    ...
    # Hint: kernel_matrix or kernel_dot may be useful here
    ###################################################################################
    support_alpha = alpha[is_support]
    support_y = y_train[is_support]
    support_X = X_train[is_support]
    
    # Compute kernel between X (test) and the support vectors
    K_test = kernel_matrix(X, support_X, kernel_params)  # shape (len(X), len(support_X))
    
    # h(x) = sum_{sv} (alpha_sv * y_sv * k(x_sv, x)) + b
    h = K_test @ (support_alpha * support_y) + b
    ###################################################################################
    #                                END OF YOUR CODE                                 #
    ###################################################################################
...
class CVXOPTSVC:
    ...
    Read more in https://xavierbourretsicotte.github.io/SVM_implementation.html
    # ~~END DELETE~~
