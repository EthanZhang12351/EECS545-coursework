...
def compute_softmax_probs(X: np.ndarray, W: np.ndarray) -> np.ndarray:
    ...
    ###########################################################################
    origin_vector = X @ W.T
    stable_vector = origin_vector - np.max(origin_vector, axis = 1, keepdims = True)
    exp_vector = np.exp(stable_vector)
    sum_exp_vector = np.sum(exp_vector, axis = 1, keepdims = True)
    probs = exp_vector / sum_exp_vector
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    return probs
...
def gradient_ascent_train(X_train: np.ndarray,
    ...
    for epoch in range(max_iters):
        ...
        ###################################################################
        indicator = class_matrix[int_Y_train.ravel()- 1] 
        probs = compute_softmax_probs(X_train, W)       
        diff = indicator - probs                          
        delta_W = diff.T @ X_train
        ###################################################################
        #                        END OF YOUR CODE                         #
        ###################################################################
...
def compute_accuracy(X_test: np.ndarray,
    ...
    ###########################################################################
    scores = compute_softmax_probs(X_test,W)
    pred_labels = np.argmax(scores, axis=1) + 1
    count_correct = 0
    for i in range(len(pred_labels)):
        if pred_labels[i] == int_Y_test[i]:
            count_correct+=1
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
